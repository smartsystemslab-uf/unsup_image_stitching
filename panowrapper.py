...
# Define the adjusted Sigmoid-like function S'(x, y)
def adjusted_sigmoid_edge(x, y, mu_x, mu_y, k, d0):
    D = np.sqrt((x - mu_x) ** 2 + (y - mu_y) ** 2)  # Euclidean distance from the center
    return 1 - (1 / (1 + np.exp(-k * (D - d0))))
...
class PanoWrapper:
    ...
    def pano_stitch_single_camera(
        ...
        if not self.pano_stitch_init:
            print(
                "Please, initialize the pano object by provide the calibration files."
            )
            return None
